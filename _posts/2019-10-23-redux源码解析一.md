...
首先看下`createStore`，这是redux中核心的方法，我们会使用`createStore`来创建`store`
...
//如果这两个监听器数组相等，那么浅拷贝一份nextListeners
...
整个文件的代码不是很复杂，其中有一点需要注意的，有两个数组`currentListeners`和`nextListeners`都用来存储`listener`，那为什么要用两个呢，其实在`subscribe`的注释中我们能看出一些端倪，主要是为了处理`dispatch`执行的时候，监听器的数量可能新增或者减少了，如果只用一个`currentListeners`来存储监听器，那么`dispatch`执行监听器函数时候可能会混乱，也就是同时在操作一份数据。作者用了两个数组，执行dispatch的时候把`currentListeners`指向`nextListeners`，订阅或者取消订阅的时候都会去检查这两个数组是否相等，如果相等说明在dispatch，就重新浅拷贝一份`nextListeners`,


`combineReducers` 合并`reducer`函数
...
/**该方法对传入的reducers进行遍历， 首先通过传入undefined和ActionTypes.INIT，如果返回的state是undefined那么说明没有给该reducer设置初始状态。
如果通过前面的验证，即设置了初始状态同时不为undefined，那么再传入undefined和ActionTypes.PROBE_UNKNOWN_ACTION()，如果得到的还是undefined，说明这个action type和我们reducer中匹配了，同时返回了undefined，这种情况是不允许的（其实还是没看懂）
*/
...
`bindActionCreators` 主要用于把`actionCreators`函数转换成`dispatch(action)`的函数
...
`applyMiddleware.js`使用中间件方法，该方法代码不对，但是这块是整个`redux`中最难理解的（我个人认为）
